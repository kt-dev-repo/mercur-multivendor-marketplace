...
- **Payout** — settlement to a seller's connected account, default provider Stripe Connect. Account lifecycle `PENDING` → `ACTIVE` ↔ `RESTRICTED` / → `REJECTED`, driven by provider webhooks. Payouts are reached **only** via the webhook subscriber running `createPayoutWorkflow`. **There are no scheduled jobs**: `docs/ARCHITECTURE.md` describes a daily 1 AM UTC payout job and a 15-minute capture-check job, but `packages/core` contains no `jobs/` directory and no `schedule:` anywhere — verified 2026-09-10. Do not rely on either.
